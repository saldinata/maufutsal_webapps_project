...
Class Maufutsal
{
	...
	public function checkOpenHours($court_reg,$field_code,$number_of_day,$code_cat,$start_time,$end_time)
	{
		$db = new Database();
		$count = 0;
		$register_time 	= null;
	...
	public function checkAvailableBook($current_date,$field_code,$court_reg,$start_time,$end_time)
	{
		$db 	= new Database();
		$util 	= new Utility();
		...
		$query = "SELECT * FROM tbl_booking_lapangan WHERE register=? AND code_arena=? AND court_reg=?";
		$result_booking_info = $db->getAllValue($query,[$current_date,$field_code,$court_reg]);

		foreach ($result_booking_info as $row) 
		{	
			$start_time_from_db = substr($row['jam_mulai'],0,2);
			$end_time_from_db = substr($row['jam_akhir'],0,2);
			$result_compare = $this->getPositionTime($start_time,$end_time,$start_time_from_db,$end_time_from_db);

			$data_tracker[$index_tracker] = $this->getTemporaryCondition($result_compare[0],$result_compare[1]);
			$index_tracker++;
		}
	...
	public function getPositionTime($start_time,$end_time,$start_time_from_db,$end_time_from_db)
	{
		$respons = null;
		...
		else if($start_time<$start_time_from_db)
		{
			$respons[0]	= '1';
			...
			else if($end_time<$start_time_from_db)
			{
				$respons[1]	= '2';
			}
			...
			else if($end_time>$end_time_from_db)
			{
				$respons[1]	= '7';
			}
			else if($end_time==$end_time_from_db)
		...
		else if($start_time>$end_time_from_db)
		{
			$respons[0]	= '6';
	...
	public function storeonlinebookbytransfer($methodpayment,$type_transfer)
	{
		...
		$result_store_booking = $db->insertValue($query,[$current_date,$date_time,$register,$court_reg,$arena_name,$field_code,$code_cat,$start_time,$end_time,$source,$code_book,$verification,$id_user_member,$cost,$friendly_match,$methodpayment,$type_transfer,$account_no,$account_name,$account_id]);
	...
	public function storecompetition($methodpayment,$type_transfer)
	{
		...
		$id_reg_member	= $this->getIdRegMember($util->decode($_COOKIE['maufutsal_dat']));
		$id_user_member = $util->decode($_COOKIE['maufutsal_dat']);
		...
		$result_store_competition = $db->insertValue($query,[$current_date,$name_team,$id_reg_member,$id_competition,$payment,$methodpayment,$type_transfer,$bank_name,$account_no,$account_name,$agent_code_payment,$verification,$code_reg,$lolos,$usage_member,$payment_code,$bank_code,$signature,$booking_datetime,$date_time]);
	...
	public function storeslamevent($methodpayment,$type_transfer)
	{
		...
		$query = "INSERT INTO tbl_member_kompetisi(tanggal,nama_team,id_reg_member,id_kompetisi,payment,payment_method,type_transfer,bank_name,account_no,account_name,agent_code_payment,verification,code_reg,lolos,usage_member,payment_code,bank_code,signature,booking_datetime,date_time,field_list,lawan_tanding,futsal_tanding,lapangan_tanding,tanggal_tanding,pukul_tanding) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

		$result_store_slam_event = $db->insertValue($query,[$current_date,$name_team,$id_reg_member,$id_competition,$payment,$methodpayment,$type_transfer,$bank_name,$account_no,$account_name,$agent_code_payment,$verification,$code_reg,$lolos,$usage_member,$payment_code,$bank_code,$signature,$booking_datetime,$date_time,$field_data,$lawan_tanding,$futsal_tanding,$lapangan_tanding,$tanggal_tanding,$pukul_tanding]);
